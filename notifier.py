...
def format_job(job: Job) -> str:
    """
    Formats a Job object into an HTML-styled message for Telegram.

    All user/scraper-sourced text fields are HTML-escaped before being
    interpolated into the message, since Telegram is called with
    parse_mode="HTML" and unescaped '<', '>', '&' or '"' characters would
    either break message delivery or allow unintended markup injection.
    """
    title = html.escape(job.title or "Government Job Opening")
    org = html.escape(job.organisation or "Government Organisation")
    qualification = html.escape(job.qualification or "B.E / B.Tech / Diploma / Any Graduate")
    vacancies = html.escape(job.vacancies or "As per notification")
    last_date = html.escape(job.last_date or "Refer Official PDF")
    salary = html.escape(job.salary or "Standard Pay Scale")
    job_type = html.escape(job.job_type or "Government")

    apply_link = html.escape(resolve_job_portal(job), quote=True)
    raw_pdf_link = job.notification_pdf if (job.notification_pdf and job.notification_pdf.startswith("http")) else resolve_job_portal(job)
    pdf_link = html.escape(raw_pdf_link, quote=True)
    ...
    if pdf_link and pdf_link != apply_link:
        lines.append(f'📄 <a href="{pdf_link}"><b>Download Official PDF Notification</b></a>')

    lines.append("━━━━━━━━━━━━━━━━━━")
    lines.append("🤖 <i>AI Powered Govt Jobs Alert</i>")
