...
class Database:
    ...
    def get_connection(self):
        ...
        NOTE: `with sqlite3.connect(...) as conn:` (the previous implementation)
        does NOT close the connection on exit - it only commits/rolls back the
        transaction. In a long-running process (the Telegram polling bot),
        that leaked one open file handle per query. This wrapper commits on
        success, rolls back on error, and always closes the connection.

        journal_mode is intentionally left at SQLite's default (rollback
        journal) rather than WAL: this project persists jobs.db by committing
        it back to the Git repo from GitHub Actions, and WAL mode leaves
        behind extra -wal/-shm side files that would need to be committed
        (and reconciled) too.
        """
